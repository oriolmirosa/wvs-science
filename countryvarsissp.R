...
countryindicatorsissp <- read.csv("countryyearissp.csv", header = TRUE)

countryindicatorsissp$gdp <- NA
countryindicatorsissp$gdpyear <- NA
countryindicatorsissp$gdpsource <- ""

countryindicatorsissp$governance <- NA
countryindicatorsissp$govyear <- NA
countryindicatorsissp$govsource <- ""

countryindicatorsissp$gini <- NA
countryindicatorsissp$giniyear <- NA
countryindicatorsissp$ginisource <- ""

countryindicatorsissp$polstab <- NA
countryindicatorsissp$polstabyear <- NA
countryindicatorsissp$polstabsource <- ""

countryindicatorsissp$mleftright <- NA
countryindicatorsissp$mleft <- NA
countryindicatorsissp$mright <- NA
countryindicatorsissp$mideologyyear <- NA
countryindicatorsissp$ideologysource <- ""
...
# Let's add the GDP per capita information to countryindicatorsissp

nodatagdp <- vector(mode = "character", 0)

for(row in 1:nrow(countryindicatorsissp)) {
  country <- countryindicatorsissp[row, 1]
  year <- countryindicatorsissp[row, 2]
  if(sum(gdp$country %in% country) == 0) {
    nodatagdp <- c(nodatagdp, levels(country)[country])
    next
  }
  if(is.na(countryindicatorsissp[row, 3])) {
    if(!is.na(gdp$NY.GDP.PCAP.PP.KD[gdp$country == country & gdp$year == year])) {
      countryindicatorsissp[row, 3] <- gdp$NY.GDP.PCAP.PP.KD[gdp$country == country & gdp$year == year]
      countryindicatorsissp[row, 4] <- year
      countryindicatorsissp[row, 5] <- "WDI"
    } else {
      values <- gdp$NY.GDP.PCAP.PP.KD[gdp$country %in% country]
      years <- gdp$year[gdp$country %in% country]
      nas <- !is.na(values)
      if(sum(nas) == 0) {
        countryindicatorsissp[row, 3] <- NA
        countryindicatorsissp[row, 4] <- NA
      } else {
        values <- values[nas]
        years <- years[nas]
        dist <- abs(years - year)
        countryindicatorsissp[row, 3] <- values[which.min(dist)]
        countryindicatorsissp[row, 4] <- years[which.min(dist)]
        countryindicatorsissp[row, 5] <- "WDI" 
      }
    }
  }
...
# Here we add the GINI information to countryindicatorsissp
...
for(row in 1:nrow(countryindicatorsissp)) {
  ...
  if(is.na(countryindicatorsissp[row, 9])) {
    if(!is.na(gini$SI.POV.GINI[gini$country == country & gini$year == year])) {
      countryindicatorsissp[row, 9] <- (gini$SI.POV.GINI[gini$country == country & gini$year == year])/100
      countryindicatorsissp[row, 10] <- year
      countryindicatorsissp[row, 11] <- "WDI"
    } else {
      values <- gini$SI.POV.GINI[gini$country %in% country]
      years <- gini$year[gini$country %in% country]
      nas <- !is.na(values)
      if(sum(nas) == 0) {
        countryindicatorsissp[row, 9] <- NA
        countryindicatorsissp[row, 10] <- NA
      } else {
        values <- values[nas]
        years <- years[nas]
        dist <- abs(years - year)
        countryindicatorsissp[row, 9] <- (values[which.min(dist)])/100
        countryindicatorsissp[row, 10] <- years[which.min(dist)]
        countryindicatorsissp[row, 11] <- "WDI" 
      }
    }
  }
...
countryindicatorsissp$gini[countryindicatorsissp$country == "South Korea" & countryindicatorsissp$year == 2010] <- 0.307
countryindicatorsissp$giniyear[countryindicatorsissp$country == "South Korea" & countryindicatorsissp$year == 2010] <- 2012
countryindicatorsissp$ginisource[countryindicatorsissp$country == "South Korea" & countryindicatorsissp$year == 2010] <- "OECD"

countryindicatorsissp$gini[countryindicatorsissp$country == "New Zealand" & countryindicatorsissp$year == 2010] <- 0.323
countryindicatorsissp$giniyear[countryindicatorsissp$country == "New Zealand" & countryindicatorsissp$year == 2010] <- 2011
countryindicatorsissp$ginisource[countryindicatorsissp$country == "New Zealand" & countryindicatorsissp$year == 2010] <- "OECD"
...
# Now we add the government effectiveness information to countryindicatorsissp
...
for(row in 1:nrow(countryindicatorsissp)) {
  ...
  if(is.na(countryindicatorsissp[row, 6])) {
    if(!is.na(gov[gov$country == levels(country)[country], cols[4]])) {
      countryindicatorsissp[row, 6] <- gov[gov$country == levels(country)[country], cols[4]]
      countryindicatorsissp[row, 7] <- year
      countryindicatorsissp[row, 8] <- "WGI"
    } else {
      countryindicatorsissp[row, 6] <- NA
      countryindicatorsissp[row, 7] <- NA
    }
  }
}
...
# Now we add the political stability information to countryindicatorsissp
...
for(row in 1:nrow(countryindicatorsissp)) {
  ...
  if(is.na(countryindicatorsissp[row, 12])) {
    if(!is.na(polstab[polstab$country == levels(country)[country], cols[4]])) {
      countryindicatorsissp[row, 12] <- polstab[polstab$country == levels(country)[country], cols[4]]
      countryindicatorsissp[row, 13] <- year
      countryindicatorsissp[row, 14] <- "WGI"
    } else {
      countryindicatorsissp[row, 12] <- NA
      countryindicatorsissp[row, 13] <- NA
    }
  }
}
...
for(row in 1:nrow(countryindicatorsissp)) {
  ...
  if(is.na(countryindicatorsissp[row, 15])) {
    if(length(ideology$mleftright[ideology$countryname == country & ideology$year == year]) != 0) {
      if(!is.na(ideology$mleftright[ideology$countryname == country & ideology$year == year])) {
        countryindicatorsissp[row, 15] <- ideology$mleftright[ideology$countryname == country & ideology$year == year]
        countryindicatorsissp[row, 16] <- ideology$mleft[ideology$countryname == country & ideology$year == year]
        countryindicatorsissp[row, 17] <- ideology$mright[ideology$countryname == country & ideology$year == year]
        countryindicatorsissp[row, 18] <- year
        countryindicatorsissp[row, 19] <- "MP"
        next
      }
    } else {
      ...
      nas <- !is.na(values)
      if(sum(nas) == 0) {
        countryindicatorsissp[row, 15] <- NA
        countryindicatorsissp[row, 16] <- NA
        countryindicatorsissp[row, 17] <- NA
        countryindicatorsissp[row, 18] <- NA
      } else {
        values <- values[nas]
        values2 <- values2[nas]
        values3 <- values3[nas]
        years <- years[nas]
        dist <- abs(years - year)
        countryindicatorsissp[row, 15] <- values[which.min(dist)]
        countryindicatorsissp[row, 16] <- values2[which.min(dist)]
        countryindicatorsissp[row, 17] <- values3[which.min(dist)]
        countryindicatorsissp[row, 18] <- years[which.min(dist)]
        countryindicatorsissp[row, 19] <- "MP" 
      }
    }
  }
...
# Finally, we save countryindicatorsissp so that it can be loaded in the main
# script and used to add the values to the main data frame with the World
# Values Survey data

save(countryindicatorsissp, file = "countryindicatorsissp.Rda")
